...
static SS_TIME_TYPE spec_create_vector_rt[SS_TOTAL_REGS];
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
      ...
      if (RUU[RUU_head].ea_comp)
	{
	  ...
	  if ((SS_OP_FLAGS(LSQ[LSQ_head].op) & (F_MEM|F_STORE))
	      == (F_MEM|F_STORE))
	    {
		  ...
		  if (RSLINK_VALID(olink))
		      ...
		      if (OPERANDS_READY(olink->rs))
			{
			  /* yes! enqueue instruction as ready, NOTE: stores
			     ...
			     them */
			  if (!olink->rs->in_LSQ
			      || ((SS_OP_FLAGS(olink->rs->op)&(F_MEM|F_STORE))
				  == (F_MEM|F_STORE)))
...
lsq_refresh(void)
{
  ...
  for (i=0, index=LSQ_head, n_std_unknowns=0;
	  ...
	  (SS_OP_FLAGS(LSQ[index].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
	{
      ...
      if (/* load? */
	  ((SS_OP_FLAGS(LSQ[index].op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD))
...
ruu_issue(void)
{
  ...
  for (n_issued=0;
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
	      && ((SS_OP_FLAGS(rs->op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE)))
	    {
	      ...
	      if (SS_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu)
			...
			panic("functional unit already in use");
		      ...
		      if (rs->in_LSQ
			  && ((SS_OP_FLAGS(rs->op) & (F_MEM|F_LOAD))
			      == (F_MEM|F_LOAD)))
			{
			  int events = 0;
			  ...
			  if (i != LSQ_head)
			      ...
			      for (;;)
				{
				  ...
				  if ((SS_OP_FLAGS(LSQ[i].op) & F_STORE)
				      && (LSQ[i].addr == rs->addr))
				    {
				      /* hit in the LSQ */
				      load_lat = 1;
...
#define SET_HI(EXPR)            (spec_mode                              \
				 ? (use_spec_HI=TRUE,                   \
				    spec_regs_HI = (EXPR))              \
				 : (regs_HI = (EXPR)))
#define LO                      (use_spec_LO ? spec_regs_LO : regs_LO)
#define SET_LO(EXPR)            (spec_mode                              \
				 ? (use_spec_LO=TRUE,                   \
				    spec_regs_LO = (EXPR))              \
				 : (regs_LO = (EXPR)))
#define FCC                     (use_spec_FCC ? spec_regs_FCC : regs_FCC)
...
ruu_dispatch(void)
{
  ...
  SS_ADDR_TYPE next_PC, target_PC;	/* actual next/target PC address */
  ...
  struct bpred_update *dir_update_ptr;	/* branch predictor dir update ptr */
  ...
  unsigned int temp_uint;		/* temp variable for spec mem access */
...
#undef CONNECT
      ...
      if (SS_OP_FLAGS(op) & F_MEM)
	{
	  ...
	  || ((SS_OP_FLAGS(op) & (F_CTRL|F_DIRJMP)) == (F_CTRL|F_DIRJMP)
      ...
      if (op != NOP)
	{
	  ...
	  if (SS_OP_FLAGS(op) & F_MEM)
	      ...
	      if (OPERANDS_READY(rs))
		{
	      ...
	      if (((SS_OP_FLAGS(op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
		  && OPERANDS_READY(lsq))
		{
      ...
      if (!spec_mode)
	{
      ...
      ptrace_newstage(pseq, PST_DISPATCH,
		      (pred_PC != next_PC) ? PEV_MPOCCURED : 0);
      if (op == NOP)
	{
	  /* end of the line */
	  ptrace_endinst(pseq);
