 ...
 *
 * $Id: range.c,v 1.1 1997/03/11 01:32:52 taustin Exp taustin $
 *
 * $Log: range.c,v $
		...
		struct range_pos_t *pos)	/* position return buffer */
{
  ...
  /* get position value */
  errno = 0;
  pos->pos = (SS_COUNTER_TYPE)strtoul(s, &endp, /* parse base */0);
  if (!errno && !*endp)
    {
      /* good conversion */
      return NULL;
  ...
  errno = 0;
  pos->pos = (SS_COUNTER_TYPE)strtod(s, &endp);
  if (!errno && !*endp)
    {
      /* good conversion */
  ...
  if (sym != NULL)
    {
      pos->pos = (SS_COUNTER_TYPE)sym->addr;
      return NULL;
    }
		...
		FILE *stream)			/* output stream */
{
  switch (pos->ptype)
    {
    case pt_addr:
      fprintf(stream, "@0x%08x", (unsigned int)pos->pos);
      break;
    case pt_inst:
  ...
  *p = '\0';
  ...
  if (*pos2)
  ...
  else
    {
      ...
      range->end.ptype = range->start.ptype;
#ifdef __GNUC__
      range->end.pos = 0xffffffffffffLL;
#else /* !__GNUC__ */
		 ...
		 SS_COUNTER_TYPE cycle)		/* cycle count */
{
  ...
  switch (range->start.ptype)
    {
    case pt_addr:
      if (addr < range->start.pos)
	return /* before */-1;
      else if (range->start.pos <= addr && addr <= range->end.pos)
	return /* inside */0;
