 ...
 *
 * $Id: cache.c,v 1.4 1997/03/11 01:08:30 taustin Exp taustin $
 *
 * $Log: cache.c,v $
	     ...
	     SS_ADDR_TYPE *repl_addr)	/* for address of replaced block */
{
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      lat += MAX(0, (int)(repl->ready - now));
 
      /* stall until the bus to next level of memory is available */
      lat += MAX(0, (int)(cp->bus_free - (now + lat)));
 ...
 cache_hit: /* slow hit handler */
  ...
  return MAX((int)(cp->hit_latency), (int)(blk->ready - now));
