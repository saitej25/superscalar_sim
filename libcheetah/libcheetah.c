...
void ptc(unsigned long addr);
void sacnmul_woarr(unsigned long addr);
...
cheetah_init(int argc, char **argv)	/* cheetah arguments */
{
  ...
  for (i=0; i < argc; i++) {
	...
	if ((NOT strcmp ("lru", &argv[i][2])) ||
	  ...
	  fprintf (stderr,
		   "libcheetah: replacement policy `%s' not supported\n",
		   &argv[i][2]);
	  exit (1);
	}
	break;
	...
	break;
      ...
      /* -i<num> cache size intervals at which the miss ratio is desired */
      case 'i':
  ...
  case FA:
    if ((ONE << L) > MISS_RATIO_INTERVAL)
      {
        fprintf(stderr, "libcheetah: line size > output interval!!\n");
        fprintf(stderr, "libcheetah: output interval changed to line size\n");
    ...
    if (MAX_CACHE_SIZE < (ONE << L))
      {
        fprintf(stderr, "libcheetah: max cache size is less than line size\n");
        exit (1);
      }
    else
      {
	  ...
	  fprintf(stderr, "libcheetah: warning: mem limit may be exceeded\n");
      }
  ...
  case DM:
    if (A > B)
      {
        fprintf (stderr, "libcheetah: min line size greater than max line\n");
        exit (1);
      }
    if (B > C)
      {
        fprintf (stderr, "libcheetah: max line size greater than cache size\n");
        exit (1);
      }
