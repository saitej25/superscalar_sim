...

{
	 ...
	 set2_match;		/* Scratch */
     int l, nr;			/* ints for input buffering */
...
{   int i, depth, k;
   ...
   TAG_SET1_MASK = ((ONE << MAX_LINE_SZ) - ONE);
    ...
    if ((lm_arr = (unsigned *) calloc ((ONE << (NO_LINES - 1)), sizeof(int))))
      ;
    else
      goto error;
    if ((hits = (unsigned *) calloc((MAX_LINE_SZ+1), sizeof(int))))
      ;
    else
      goto error;
